...
def main():
    ...
    for key in sorted(list(mpath.keys()), reverse=True):
        ...
        try:
            ...
            xpub = bip32.serialize(client.get_public_node(bip32_path).node, ( 0x0488B21E if MAINNET else 0x043587CF ))
            ...
            for i in range(max_gab):
                ...
                if not HEADER_PRINTED:
                    if key == 'masternode':
                        ...
                        print ('**** ====> use following address for trezor/keepkey wallet(default path)')
                ...
                assert bip32_address == address, "address mismatch, bip32 : %s <--> hw : %s" % (bip32_address, address)
                assert publicnode ==  bip32_addrpkey, "pubkey mismatch, bip32 : %s <--> hw : %s" % (bip32_addrpkey, publicnode)              
            ...
            err_msg = str(e.args)
            print_err_exit(get_caller_name(), get_function_name(), err_msg, None, None)
            ...
            print_err_exit(get_caller_name(), get_function_name(), "KeyboardInterrupt", None, None)
